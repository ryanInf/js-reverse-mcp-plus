...
JavaScript 逆向工程 MCP 服务器，让你的 AI 编码助手（如 Claude、Cursor、Copilot）能够调试、分析**并重写**网页中的 JavaScript 代码。
...
- **JS 覆写**：三层覆写能力 —— 页面加载前注入（`inject_before_load`）、运行时脚本编辑（`override_script`）、网络层响应拦截（`intercept_response`）
...
| `intercept_response`     | 拦截并修改网络响应（静态替换或 JS transform）⚠️ 会激活 CDP Fetch 域，可能被反检测探测 |

### JS 覆写

| 工具                   | 描述                                                                                          |
| ---------------------- | --------------------------------------------------------------------------------------------- |
| `inject_before_load`   | 在页面加载前注入 JS（`Page.addScriptToEvaluateOnNewDocument`），每次导航/刷新自动执行，持久化直到移除 |
| `override_script`      | 运行时替换已加载脚本的 V8 源码（`Debugger.setScriptSource`），立即生效，可选导航后自动重应用 |
| `intercept_response`   | 网络层拦截响应并修改（`Fetch.enable`），支持静态替换或 JS transform 函数 |
...
**拦截加密参数** — 在加密函数加载前 monkey-patch：

```
用 inject_before_load 注入一段 JS，拦截 window.encrypt 的参数
```

**运行时修改脚本** — 移除 debugger 语句或检测逻辑：

```
用 override_script 搜索目标脚本中的反调试代码并替换为空
```

**网络层替换** — 替换整个 JS 文件的响应内容：

```
用 intercept_response 拦截某个 JS 文件的加载，替换为修改后的版本
```
